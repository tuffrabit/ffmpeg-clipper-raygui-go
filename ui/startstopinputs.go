...
func StartStopInputs(appState *state.AppState) error {
	...
	if gui.TextBox(startStopInputsStartInputRect, &appState.ClipState.Start, 9, appState.ClipState.StartEditMode) {
		appState.ClipState.StartEditMode = !appState.ClipState.StartEditMode
	}

	gui.Label(startStopInputsEndLabelRect, "End (hh:mm:ss)")
	if gui.TextBox(startStopInputsEndInputRect, &appState.ClipState.End, 9, appState.ClipState.EndEditMode) {
		appState.ClipState.EndEditMode = !appState.ClipState.EndEditMode
	}
