...
func ClipIntelH264(video string, newVideoName string, startTime string, endTime string, profileState state.ProfileState) (string, error) {
	cmd := exec.Command("ffmpeg",
		...
		"h264_qsv",
		"-preset",
		profileState.Profile.EncoderSettings.IntelH264.EncodingPreset,
		"-global_quality",
		profileState.IntelH264EncodingQualityTargetInput,
		"-look_ahead",
		"1",
		"-vf",
...
func InitClipIntelH264Cmd(video string, newVideoName string, startTime string, endTime string, profileState state.ProfileState) (*exec.Cmd, context.CancelFunc) {
